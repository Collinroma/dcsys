...
def euclideanDistance(point, point1):
    distance = math.sqrt((point1[0] - point[0])**2 + (point1[1] - point[1])**2)
    return distance
...
def getFacialRatios(img, landmarks):
    # Right eyes
    rh_right = landmarks[33]
    rh_left = landmarks[163]
    rv_top = landmarks[7]
    rv_bottom = landmarks[145]

    # LEFT_EYE
    lh_right = landmarks[362]
    lh_left = landmarks[380]
    lv_top = landmarks[381]
    lv_bottom = landmarks[385]
    ...
    if lvDistance != 0 and lhDistance !=0:
        ...
        blinkRatio = (reRatio+leRatio)/2
    else:
        blinkRatio = 0

    lip_right = landmarks[61]
    lip_left = landmarks[91]
    lip_top = landmarks[40]
    lip_bottom = landmarks[57]

    lipDistance = euclideanDistance(lip_top, lip_bottom)

    return blinkRatio, lipDistance
...
def gen_frames():
    ...
    # Initialize mixer and audio files
    mixer.init()
    voice_left = mixer.Sound('punch.wav')
    voice_right = mixer.Sound('punch.wav')
    voice_down = mixer.Sound('punch.wav')
    eyes_blink= mixer.Sound('punch.wav')
    yawn = mixer.Sound('punch.wav')
    ...
    # Initialize variables for head and eyes movement
    prev_nose_2d = None
    prev_eye_2d = None
    head_moved = False
    eyes_moved = False
            ...
            for face_landmarks in results.multi_face_landmarks:
                ...
                if y< -10:
                    ...
                    if Counter_left % Threshold_Frame[counter_left] == 0  and pygame.mixer.get_busy()==0:
                        ...
                        voice_left.play()
                        alert_count += 1  # Increment alert count variable
                        print("Alert count:", alert_count)  # Print alert count value
                ...
                if results.multi_face_landmarks:
                    ...
                    colorBackgroundText(image,  f'Looking left for: {Counter_left} frames', FONTS, 0.6, (10,60),2, LIGHTBLUE, WHITE)
                    colorBackgroundText(image,  f'Looking right for: {Counter_right} frames', FONTS, 0.6, (350,60),2, LIGHTBLUE, WHITE)
                    colorBackgroundText(image,  f'Looking Down for : {Counter_down} frames', FONTS, 0.6, (10,90),2, LIGHTBLUE, WHITE)
...
def get_alert_count(request):
    # Get the alert count value from wherever it is stored
    result = alert_count  # Replace with actual value

    # Return the alert count as a JSON response
    return JsonResponse({'alert_count': result})